...
public:
    int maxArea(vector<int>& height) {
        ...
        set<int, greater<> > st;
        pair<int, int> large_v_idx = make_pair(n*2, -1); // left, right
        for (int i = 0; i < n; i++) {
            st.insert(height[i]);
        ...
        for (int h : st) {
            ...
            for (int idx : val2idx[h]) {
                large_v_idx.first = min(large_v_idx.first, idx);
                large_v_idx.second = max(large_v_idx.second, idx);
            }
            int l = large_v_idx.first, r = large_v_idx.second; // leftest idx height > h, righest...
