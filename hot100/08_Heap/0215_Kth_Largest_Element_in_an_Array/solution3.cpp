...
private:
    int quickSelect(std::vector<int>& a, int l, int r, int t) {
        if (l == r) return a[l];

        int pivot = a[l + rand() % (r - l + 1)];
        int lit = l, rit = r; // [l, lit) < p, (rit, r] > p
        int i = l;

        // check [1,rit]
        while (i <= rit) {
            if (a[i] < pivot) {
                swap(a[lit], a[i]); // move  a[lit] <- a[i]
                i++; lit++;
            } else if (a[i] > pivot) {
                swap(a[i], a[rit]); // move  a[i] -> a[rit]
                rit--;
            } else { // eq
                i++;
            }
        ...
        if (lit <= t && t <= rit) { // equal
            return pivot;
        } else if (t < lit) {
            return quickSelect(a, l, lit-1, t);
        } else { // rit < t
            return quickSelect(a, rit+1, r, t);
        }
